def suck_it()
    ...
    sample_counter = 0                  # Sample counter. Counts four samples. This needs to come from main loop
    ...
    while in_position:                  # in_position variable checks location of USV against the target GPS location
        if flushed_flag:
            sample = samplePins[sample_counter]
            GPIO.out(pump_pin, LOW)  # Turn the pump on
            GPIO.out(sample, LOW)    # Turn the specific solenoid on given by sample variable
            while finished_pump:
