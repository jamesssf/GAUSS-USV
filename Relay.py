...
# There are 8 relays
# Relay 1 -> GPIO 17 - 
# Relay 2 -> GPIO 27 - Flush Solenoid
# Relay 3 -> GPIO 15 - Pump Solenoid
# Relay 4 -> GPIO 16 - Solenoid 1
# Relay 5 -> GPIO 13 - 
...
flushPin = 27  # Relay connected to the solonoid for flushing the system
pumpPin = 15  # Relay connected to the peristaltic pump
pinList = [13, 15, 16, 19, 22, 26]  # Used for initializing the relays below
...
def pump_on():
    GPIO.OUT(pumpPin, GPIO.LOW)  # Turn the pump on

def pump_off():
    GPIO.OUT(pumpPin, GPIO.HIGH)  # Turn the pump off

def solenoid_on(sample_pin):
    GPIO.OUT(sample_pin, GPIO.LOW)  # Turn the solenoid on

def solenoid_off(sample_pin):
    GPIO.OUT(sample_pin, GPIO.HIGH)  # Turn the solenoid off

def flush_on():
    GPIO.OUT(flushPin, GPIO.LOW)  # Turn the flush solenoid on

def flush_off():
    GPIO.OUT(flushPin, GPIO.HIGH)  # Turn the flush solenoid off
